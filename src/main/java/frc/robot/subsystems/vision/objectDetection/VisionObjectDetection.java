...
public class VisionObjectDetection extends SubsystemBase {
    private final VisionObjectDetectionInputsAutoLogged objectDetectionCameraInputs = new VisionObjectDetectionInputsAutoLogged();
    private final VisionObjectDetectionIO visionObjectDetectionIO;
    private final String hostname;
    private final Transform3d robotCenterToCamera;
    private Drive drive;

    public VisionObjectDetection(String hostname, Transform3d robotCenterToCamera, Drive drive, VisionObjectDetectionIO visionObjectDetectionIO) {
    ...
    public Rotation3d[] getTargetObjectsRotations(SimulatedGamePieceConstants.GamePieceType targetGamePiece) {
        if (targetGamePiece == SimulatedGamePieceConstants.GamePieceType.CORAL && Constants.VisionObjectDetection.SHOULD_IGNORE_LOLLIPOP_CORAL) {
    ...
    private Translation2d calculateObjectPositionFromRotation(Rotation3d objectRotation) {
        final Pose2d robotPoseAtResultTimestamp = drive.getPoseAtTimestamp(objectDetectionCameraInputs.latestResultTimestamp);
    ...
    private boolean isLollipop(Rotation2d objectYaw) {
        for (Rotation3d algaeYaw : getTargetObjectsRotations(SimulatedGamePieceConstants.GamePieceType.ALGAE)) {
            ...
            if (difference < Constants.VisionObjectDetection.LOLLIPOP_TOLERANCE.getDegrees())
                return true;
        }
        return false;
    }
