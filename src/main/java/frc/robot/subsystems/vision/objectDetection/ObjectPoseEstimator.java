...
public class ObjectPoseEstimator extends SubsystemBase {
  ...
  private final VisionObjectDetection camera;
   ...
   *
   * @param deletionThresholdSeconds the time in seconds after which an object is considered old and
   *     removed
  ...
  /**
   * Updates the object positions based on the camera detected objects. Removes objects that have
   * not been detected for {@link ObjectPoseEstimator#deletionThresholdSeconds}.
   */
  @Override
  public void periodic() {
    ...
    removeOldObjects();
    Logger.recordOutput(
        "ObjectPoseEstimator/knownObjectPositions",
        getObjectsOnField().toArray(Translation2d[]::new));
  }
  ...
  public void removeClosestObjectToIntake(Transform2d intakeTransform) {
    final Pose2d robotPose = RobotContainer.getSuperstructure().getEstimatedRobotPose();
    removeObject(
        getClosestObjectToPosition(robotPose.transformBy(intakeTransform).getTranslation()));
  }
  ...
  public void removeClosestObjectToPose(Pose2d fieldRelativePose) {
    final Translation2d closestObject =
        getClosestObjectToPosition(fieldRelativePose.getTranslation());
    removeObject(closestObject);
  }
   ...
   *
   * @param objectPosition the position of the object to be removed. Must be the precise position as
   *     stored in the pose estimator.
   */
  ...
  public Translation2d getClosestObjectToRobot() {
    return getClosestObjectToPosition(
        RobotContainer.getSuperstructure().getEstimatedRobotPose().getTranslation());
  }
  ...
  public Translation2d getClosestObjectToPosition(Translation2d position) {
    final Translation2d[] objectsTranslations =
        knownObjectPositions.keySet().toArray(Translation2d[]::new);
    if (knownObjectPositions.isEmpty()) return null;
  ...
  /**
   * Calculates the "distance rating" of an object. The "distance rating" is a unit used to
   * calculate the distance between 2 poses. It factors in both translation and rotation differences
   * by scaling the units depending on the {@link DistanceCalculationMethod}.
   *
  ...
  private double calculateObjectDistanceRating(Translation2d objectTranslation, Pose2d pose) {
    ...
    final double rotationDifferenceDegrees =
        Math.abs(pose.getRotation().getDegrees() - Math.atan2(yDifference, xDifference));
    return translationDifference * rotationToTranslation
        + rotationDifferenceDegrees * (1 - rotationToTranslation);
  }
  ...
  private void updateObjectPositions() {
    ...
    for (Translation2d visibleObject : camera.getObjectPositionsOnField(gamePieceType)) {
      ...
      if (closestObjectToVisibleObjectDistanceMeters
              < Constants.VisionObjectDetection.TRACKED_OBJECT_TOLERANCE_METERS
  ...
  public enum DistanceCalculationMethod {
    ...
    /**
     * The ratio of rotation to translation in the distance rating calculation. A value of 0 means
     * only rotation is considered, 1 means only translation is considered. Values in between are
     * the ratio of rotation to translation in the distance rating calculation. For example, a value
     * of 0.1 means that 9 cm of translation is considered equivalent to 1 degree of rotation.
     */
