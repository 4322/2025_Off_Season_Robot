...
public class DeltaDebouncer {
    public enum Mode {
        INSTANT, // Only trigger if a single change is greater than the threshold; Checks the current value against the last value calculated
    ...
    private final double debounceTimeSeconds; // Amount of time to wait when the delta exceeds the threshold before returning true
    private final double maxAccumulationSeconds; // Only used in cumulative mode; Amount of time to accumulate changes before resetting the baseline value
    ...
    public DeltaDebouncer(double debounceTimeSeconds, double deltaThreshold, Mode mode, double maxAccumulationSeconds, ChangeType changeType) {
    ...
    public boolean calculate(double value) {
        ...
        switch (changeType) {
            ...
            case DECREASE:
                ...
                break;
            case BOTH:
                correctChange = Math.abs(delta) >= deltaThreshold; // Trigger for both positive and negative deltas
                break;
        }
        ...
        if (correctChange) {
            ...
            // If the timer has elapsed the debounce time (due to the delta being constantly above the threshold), we return true
