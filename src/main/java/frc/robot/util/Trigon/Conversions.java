...
public class Conversions {
   ...
   * @param gearRatio the gear ratio between the motor and the system. 2 means that 2 motor
   *     rotations are 1 system rotation.
  ...
  /**
   * Applies an offset to the position read from the motor in order to compensate for a sensor
   * offset.
   *
   * @param position the target position of the motor.
   * @param offset the encoder value when the system is on zero position.
   ...
   * The voltage compensation saturation determines what voltage represents 100% output. The
   * compensated power is the voltage represented by a percentage of the saturation voltage.
   *
  ...
  /**
   * Converts a target output percentage output to voltage when voltage compensation is enabled. The
   * voltage compensation saturation determines what voltage represents 100% output. The compensated
   * power is the voltage represented by a percentage of the saturation voltage.
   *
  ...
  public static String snakeCaseToCamelCase(String input) {
    ...
    for (String part : parts) {
      ...
      camelCase
          .append(Character.toUpperCase(lowerCasePart.charAt(0)))
          .append(lowerCasePart.substring(1));
    }
