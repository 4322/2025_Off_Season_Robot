...
/**
 * A class that handles writing, deleting, and renaming files. This class also holds the
 * "DEPLOY_PATH" constant, which is the absolute path of the deploy folder.
 */
public class FilesHandler {
  ...
  /**
   * Creates a file using a safe method of writing. This method will write the string to a temporary
   * file, delete the original file, and rename the temporary file to the desired name.
   *
   * @param absolutePath the file's absolute path
   * @param str the string to write to the file
  ...
  /**
   * Reads a file and returns its content as a string. If the file does not exist, it will check for
   * a .tmp file.
   *
   * @param absolutePath the file's absolute path
  ...
  public static String readFile(String absolutePath) throws IOException {
    final String newPath =
        !fileExists(absolutePath) && fileExists(absolutePath + ".tmp")
            ? absolutePath + ".tmp"
            : absolutePath;
  ...
  private static String extractFileNameFromAbsolutePath(String absolutePath) {
    ...
    if (absolutePath.endsWith("/")) slashIndex = absolutePath.lastIndexOf("/", slashIndex - 1);
