...
import frc.robot.util.Trigon.FieldConstants;
import frc.robot.util.Trigon.flippable.FlippablePose3d;
import frc.robot.util.Trigon.flippable.FlippableTranslation2d;
import java.util.ArrayList;
import java.util.List;

public class SimulatedGamePieceConstants {
  public static final double G_FORCE = 9.806;
  public static final double CORAL_INTAKE_TOLERANCE_METERS = 0.3,
  ...
  private static final double FIELD_WIDTH_METERS = FieldConstants.FIELD_WIDTH_METERS,
      FIELD_LENGTH_METERS = FieldConstants.FIELD_LENGTH_METERS;
  private static final Rotation3d CORAL_TO_VERTICAL_POSITION_ROTATION =
      new Rotation3d(0, Math.PI / 2, 0);

  /** Stores all the game pieces. Starts out with the game pieces the start on the field. */
      ...
      CORAL_ON_FIELD =
          new ArrayList<>(
              List.of(
                  createNewCoral(
                      new Pose3d(
                          1.22, FIELD_WIDTH_METERS / 2, 0.15, CORAL_TO_VERTICAL_POSITION_ROTATION)),
                  createNewCoral(
                      new Pose3d(
                          1.22,
                          FIELD_WIDTH_METERS / 2 - 1.83,
                          0.15,
                          CORAL_TO_VERTICAL_POSITION_ROTATION)),
                  createNewCoral(
                      new Pose3d(
                          1.22,
                          FIELD_WIDTH_METERS / 2 + 1.83,
      ...
      ALGAE_ON_FIELD =
          new ArrayList<>(
              List.of(
                  ...
                  createNewAlgae(
                      new Pose3d(1.22, FIELD_WIDTH_METERS / 2 - 1.83, 0.5, new Rotation3d())),
                  createNewAlgae(
                      new Pose3d(1.22, FIELD_WIDTH_METERS / 2 + 1.83, 0.5, new Rotation3d())),
                  createNewAlgae(
                      new Pose3d(
  ...
  private static final Rotation3d REEF_TO_2_OCLOCK_ROTATION = new Rotation3d(0, 0, Math.PI / 3),
  ...
  private static final Transform3d
      CORAL_TO_L1_ALIGNMENT =
          new Transform3d(
              new Translation3d(0, 0, 0), new Rotation3d(0, Units.degreesToRadians(15), 0)),
      CORAL_TO_L2_AND_L3_ALIGNMENT =
          new Transform3d(
              new Translation3d(0, 0, 0), new Rotation3d(0, -Units.degreesToRadians(35), 0)),
      CORAL_TO_L4_ALIGNMENT =
          new Transform3d(new Translation3d(0, 0, 0), CORAL_TO_VERTICAL_POSITION_ROTATION);
  private static final Transform3d RIGHT_BRANCH_TO_LEFT_BRANCH =
      new Transform3d(new Translation3d(0, -0.33, 0), new Rotation3d());
  public static final ArrayList<FlippablePose3d> CORAL_SCORING_LOCATIONS =
      calculatedCoralScoringLocations();
  ...
  private static FlippablePose3d calculateCoralScorePose(
      int level, int clockFace, boolean isLeftBranch) {
    ...
    return new FlippablePose3d(
        new Pose3d(new Pose2d(FieldConstants.BLUE_REEF_CENTER_TRANSLATION, new Rotation2d()))
            .transformBy(
                new Transform3d(
                    reefCenterToLevelVector.rotateBy(reefToClockFaceRotation),
                    reefToClockFaceRotation))
            .transformBy(isLeftBranch ? RIGHT_BRANCH_TO_LEFT_BRANCH : new Transform3d())
  ...
  public enum GamePieceType {
    ...
    public static String getNameFromID(int id) {
      for (int i = 0; i < values().length; i++)
        if (values()[i].id == id) return values()[i].toString();
      return "";
    }
  }
}
