...
public class SimulationScoringHandler {
  ...
  private static void checkCoralScored(SimulatedGamePiece coral) {
    final ArrayList<FlippablePose3d> scoreLocations =
        SimulatedGamePieceConstants.CORAL_SCORING_LOCATIONS;
    for (FlippablePose3d scoreLocation : scoreLocations) {
      final Pose3d flippedPose = scoreLocation.get();
      if (isGamePieceScored(
          coral, flippedPose, SimulatedGamePieceConstants.CORAL_SCORING_TOLERANCE_METERS)) {
        coral.isScored = true;
  ...
  private static void checkAlgaeScored(SimulatedGamePiece algae) {
    if (!isGamePieceScored(
        algae,
        SimulatedGamePieceConstants.PROCESSOR_LOCATION,
        SimulatedGamePieceConstants.ALGAE_SCORING_TOLERANCE_METERS)) return;

    algae.isScored = true;
    // SimulationFieldHandler.getSimulatedAlgae().remove(algae);
  }

  private static boolean isGamePieceScored(
      SimulatedGamePiece gamePiece, Pose3d scoreLocation, double scoringToleranceMeters) {
